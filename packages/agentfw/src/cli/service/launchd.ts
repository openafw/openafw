...
const LABEL = 'com.openguardrails.agentfw.daemon'
// Pre-rename labels we still know how to clean up.
const LEGACY_LABELS = ['com.agentfw.daemon', 'com.wireagentfw.daemon']
...
export async function restartLaunchd(): Promise<void> {
  ...
  if (r.exit !== 0) {
    throw new Error(
      `launchctl kickstart -k failed (exit ${r.exit}): ${r.stderr.trim()}`,
    )
  }
}
