// Credential capture. `afw wire` already reads every agent's config to
...
// routes. Captured values are stored only in ~/.afw/secrets.json (0600)
...
function authForDecoder(decoder: DecoderKind): RouteAuth {
  return decoder === 'anthropic' ? { kind: 'api-key', header: 'x-api-key' } : { kind: 'bearer' }
}
 ...
 */
export function resolveSecretInput(raw: unknown, env: Record<string, string>): string | undefined {
  if (isObj(raw)) {
...
export async function captureHermesCredentials(
  ...
  let doc: unknown
  try {
    doc = parseYaml(await readFile(opts?.configPath ?? paths.agent.hermes.config, 'utf8'))
  } catch {
    return out
  }
...
async function readOpenClawAuthProfileKey(
  ...
  const file = await readJsonFile<OpenClawAuthProfilesFile>(join(agentDir, 'auth-profiles.json'))
