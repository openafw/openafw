#!/usr/bin/env node
// agentfw-tap — stdio bridge for MCP servers spawned by wired agents.
//
...
if (sep < 0) {
  process.stderr.write(
    'agentfw-tap: missing -- separator; expected --agent X --server Y -- <cmd> [args...]\n',
  )
  process.exit(2)
}
...
if (!cmd) {
  process.stderr.write('agentfw-tap: no command specified after --\n')
  process.exit(2)
}

const AGENT = readFlag(flags, '--agent') ?? 'unknown'
const SERVER = readFlag(flags, '--server') ?? 'unknown'
const DAEMON_URL = process.env.AGENTFW_DAEMON_URL ?? 'http://localhost:9877'

const child = spawn(cmd, cmdArgs, { stdio: ['pipe', 'pipe', 'inherit'] })
if (!child.stdin || !child.stdout) {
  process.stderr.write('agentfw-tap: failed to open child stdio\n')
  process.exit(2)
}
...
child.on('error', (err) => {
  process.stderr.write(`agentfw-tap: child error: ${err.message}\n`)
  process.exit(2)
})
