// The routing policy — ~/.afw/routing.json. Per-agent decisions about
...
/** The window a budget/token cap is measured over. The calendar forms
 *  (`day`/`month`) sum usage since the local-time period start; the rolling
 *  form sums the trailing N hours — the shape a subscription's 5-hour quota
 *  needs (`{ rollingHours: 5 }`). */
 ...
 *
 *  - `budget` / `tokens`: an absolute USD or token cap over a period — the
 *    right shape for api-key upstreams, where afw sees every token.
 *  - `quota-pct`: switch when the upstream's own reported quota usage crosses a
 *    percentage — the only shape that works for subscription (OAuth) upstreams
 *    like Claude/Codex, whose absolute token budget afw can't see. The
 *    percentage is read from the provider's rate-limit response headers (see
 *    daemon/orchestrator/quota.ts).
 *  - `error`: switch on any upstream error (e.g. a 429 at the 5-hour wall). */
 ...
 *  web search, …) — and how afw fills the gap.
 *
 *  - `companion`: a side-call to another model that does support the
 *    capability — for vision, the image pre-describe pass.
 *  - `local`: afw fills the capability itself — for web_search,
 *    that's the afw-tools MCP. No model-side companion needed. */
export type CapabilityFulfillment =
  | { via: 'companion'; modelId: string; providerId?: string }
  | {
      via: 'local'
      /** Tool-provider id from ~/.afw/tool-providers.json. When set,
...
/** The set of capability ids afw understands today. Open string so
 ...
 *  per-instance suffix (`<agent>@<instance>`) minted by `afw run`;
...
function normalizePeriod(raw: unknown): SwitchPeriod | undefined {
  if (raw === 'day' || raw === 'month') return raw
  if (isObj(raw) && typeof raw.rollingHours === 'number' && raw.rollingHours > 0) {
    return { rollingHours: raw.rollingHours }
  }
  return undefined
}

export function normalizeSwitchRule(raw: unknown): SwitchRule | undefined {
  if (!isObj(raw)) return undefined
  if (raw.kind === 'error') return { kind: 'error' }
  if (raw.kind === 'budget' && typeof raw.usdLimit === 'number' && raw.usdLimit >= 0) {
    const period = normalizePeriod(raw.period)
    if (period) return { kind: 'budget', usdLimit: raw.usdLimit, period }
  }
  if (raw.kind === 'tokens' && typeof raw.tokenLimit === 'number' && raw.tokenLimit >= 0) {
    const period = normalizePeriod(raw.period)
    if (period) return { kind: 'tokens', tokenLimit: raw.tokenLimit, period }
  }
  if (
    raw.kind === 'quota-pct' &&
    typeof raw.usedPct === 'number' &&
    raw.usedPct >= 0 &&
    raw.usedPct <= 100
  ) {
    return { kind: 'quota-pct', usedPct: raw.usedPct }
  }
  return undefined
}
...
export function normalizeRouting(raw: unknown): AgentRouting | undefined {
  if (!isObj(raw)) return undefined
