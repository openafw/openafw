...
// OAuth token set in ~/.codex/auth.json under `tokens`. agentfw reads that
...
// refresh_token_reused — agentfw is a cooperative co-refresher.
//
...
export async function getCodexToken(): Promise<OAuthToken> {
  ...
  inflight = (async () => {
    ...
    const resolved = await withFileLock(lockPath, () =>
      resolveCodexToken(paths.agent.codex.auth),
    )
    cache = resolved
    return resolved
  })().finally(() => {
