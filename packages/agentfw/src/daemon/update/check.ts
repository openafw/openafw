...
const REGISTRY_URL = 'https://registry.npmjs.org/@openguardrails/agentfw/latest'
...
export async function readUpdateState(): Promise<UpdateState> {
  try {
    const parsed = JSON.parse(
      await readFile(paths.update, 'utf8'),
    ) as UpdateState
    return {
      ...parsed,
      currentVersion: VERSION,
      available: parsed.latestVersion
        ? isNewer(parsed.latestVersion, VERSION)
        : false,
    }
  } catch {
    return freshState()
 ...
 * installed one, and cache the result to ~/.agentfw/update.json. Network
