...
// No shared config is touched. Shared by `agentfw run` and the per-agent
// launchers (`agentfw claude`, `agentfw codex`).
...
async function resolveAutoCompactEnv(
  ...
  try {
    ...
    if (decision.reason === 'baseline-too-high') {
      logger.print(
        `  ↳ auto-compact left off: ${modelId}'s window compacts ~${decision.threshold} tokens, but your baseline context is ~${decision.baselineTokens} — lowering it would compact on the first prompt. Trim CLAUDE.md / MCP tools; agentfw still clamps output + flags overflow.`,
      )
    }
    return {}
  } catch {
...
export async function launchInstance(opts: LaunchInstanceOpts): Promise<void> {
  ...
  if (!opts.raw) {
    const baseUrl = agentfwUrlForInstance(wiring.agent, instanceId)
