// Surgical unwire: reverse-replay the edits agentfw recorded in a backup
...
// pointers agentfw wrote, and skips any pointer whose current value no
// longer matches what agentfw wrote (the agent or user changed it since).
...
class TomlEditor implements FileEditor {
  ...
  set(segs: string[], value: unknown): void {
    ...
    this.text =
      segs.length === 1
        ...
        : setTomlString(this.text, segs.slice(0, -1).join('.'), segs[segs.length - 1]!, value)
            .text
  }
  ...
  removeCreatedPath(segs: string[]): void {
    // agentfw owns its provider id, so the whole section it created is ours
      ...
      if (node instanceof YAMLSeq) {
        ...
        if (Number.isInteger(num) && String(num) === seg) {
          ...
          const idx = node.items.findIndex(
            (it) => it instanceof YAMLMap && it.get('name') === seg,
          )
 ...
 *  round-trip — good enough for the JSON / JSON5 configs agentfw writes,
