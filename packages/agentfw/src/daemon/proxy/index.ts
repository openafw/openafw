...
// The legacy virtual model id agentfw used to seed into agent configs
// (openclaw, hermes) before the new naming convention. Kept as a
// fallback short-circuit. New wraps use `agentfw-<agent>-<id>`.
const AGENTFW_VIRTUAL_MODEL_ID = 'agentfw'
 ...
 *   - wrap-style routes carry a agentfw-managed id (`agentfw-openclaw-main`)
 ...
 *  segment optionally carries a per-instance suffix minted by `agentfw run`:
...
export async function handleWireRequest(c: Context): Promise<Response> {
  ...
  if (!rawAgent) {
    return new Response(JSON.stringify({ error: 'agentfw: malformed route' }), {
      status: 400,
  ...
  // picker. The picker only accepts claude-* IDs, so agentfw synthesizes
  ...
  if (!route) {
    ...
    return new Response(JSON.stringify({ error: 'agentfw: no route registered', routeKey: key }), {
      status: 502,
      headers: { 'content-type': 'application/json' },
    })
  }
  ...
  // `model: "agentfw"` in the body → the route's recorded original model id.
  ...
  // agentfw-managed credentials: when wire captured an auth for this
  // route, inject agentfw's own copy (from secrets.json / OAuth store)
  // and drop whatever auth header the agent itself sent. The agent's
  // config can hold a placeholder — agentfw is the source of truth.
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        error: 'agentfw: upstream fetch failed',
