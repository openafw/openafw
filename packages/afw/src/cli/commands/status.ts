  ...
  .action(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        logger.print(`Daemon:    ${body.status} (up ${Math.round(body.uptime)}s, v${body.version})`)
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (body.driftedCount === 0) {
          ...
          logger.print('  These files drifted from a prior afw setup.')
        }
      }
    } catch {
    ...
    try {
      ...
      const retentionRaw = process.env.AFW_RETENTION_DAYS
      const retention =
        retentionRaw === '0'
          ? 'disabled'
