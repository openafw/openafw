...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'afw-restore-'))
})
...
describe('revertEntry — reverse-replay', () => {
  ...
  it('reverts codex model_provider + section, leaving agent-edited [projects] intact', async () => {
    ...
    const changes: ChangeRecord[] = [
      { type: 'set', jsonPointer: '/model_provider', from: 'openai', to: 'afw' },
      { type: 'create-path', jsonPointer: '/model_providers/afw' },
    ]
    ...
    expect(out).not.toContain('[model_providers.afw]')
