...
export const daemonCommand = new Command('daemon')
  .description('Run the agentfw daemon in the foreground, or stop/restart a running one.')
  .option('--port <port>', 'Override default port 9877.')
  .option('--log-level <level>', "'silent' | 'error' | 'warn' | 'info' | 'debug'")
  // Bare `agentfw daemon` runs in the foreground — the shape launchd/systemd
