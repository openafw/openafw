...
export type ResolvedRoute =
  ...
  | {
      ...
      /** A cheap single model for subagent/cron tasks routed to this combo —
       *  when set and the request is so classified, the orchestrator diverts to
       *  it instead of running the panel. Absent → always run the full fusion. */
      cheapModel?: ModelRef
...
export function resolveRoute(routeKey: string, decoder: DecoderKind): ResolvedRoute {
  return resolveRouteFrom(routingFor(getRoutingPolicy(), routeKey), decoder, routeKey)
}

/** Resolve an explicit `AgentRouting` (not necessarily from the policy file)
 *  into what the orchestrator should do. Drives both policy-keyed routes and
 *  afw API keys, whose target lives in keys.json. `label` is only used for
 *  log messages. */
export function resolveRouteFrom(
  ...
  if (target.kind === 'composite') {
    ...
    if (!combo) {
      logger.warn(
        `routing: ${label} → unknown combination model "${target.comboId}", passing through`,
      )
      return { kind: 'passthrough' }
    }
    return resolveFusion(label, combo, clientApi)
  }
  ...
  const capabilities = resolveCapabilityMap(label, routing.capabilities)
  ...
  for (const m of memberSpecs) {
    ...
    if (!ref) {
      ...
      logger.warn(`routing: ${label} chain member ${where} is unresolvable, skipping`)
      continue
    }
    members.push({ ...ref, switchOn: m.switchOn ?? [] })
  }
  if (members.length === 0) {
    logger.warn(`routing: ${label} chain has no resolvable members, passing through`)
    return { kind: 'passthrough' }
  }
...
function resolveFusion(
  ...
  let cheapModel: ModelRef | undefined
  if (combo.cheapModel) {
    cheapModel = resolveModelRef(combo.cheapModel.modelId, combo.cheapModel.providerId)
    if (!cheapModel)
      logger.warn(
        `routing: ${routeKey} fusion cheap model "${combo.cheapModel.modelId}" unresolvable, ` +
          'subagent/cron tasks will run the full fusion',
      )
  }
