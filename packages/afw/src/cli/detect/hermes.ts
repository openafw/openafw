...
import { type McpServerEntry, isAlreadyWrapped, wrapStdioCommand } from './mcp.ts'
import type {
  ApplyResult,
  Detection,
  Detector,
...
// only a stable handle so the user knows afw owns the model choice. Same
// pattern as OpenClaw's `AFW_VIRTUAL_MODEL.id`.
const AFW_MODEL_ID = 'afw'
...
function pickModelKey(doc: ReturnType<typeof parseDocument>, ep: PlannedEndpoint): string {
  ...
  // ep.modelId is now the wrap id `afw-hermes-<name>`, not the
...
export function pickActiveEndpoint(
  ...
  // `afw-hermes-<providerName>`. Match the provider portion.
  const expectedId = activeProvider ? `afw-${AGENT}-${activeProvider}` : ''
  const named =
    activeProvider && activeProvider !== 'auto'
      ? endpoints.find(
          (e) => e.modelId === expectedId && e.configLocation.startsWith('/custom_providers/'),
        )
      : undefined
  return named ?? endpoints.find((e) => e.configLocation === '/model/base_url') ?? endpoints[0]
}
...
export const hermesDetector: Detector = {
  ...
  async detect(): Promise<Detection | null> {
    ...
    // why: when a previous wire already pointed base_url at afw, the
    ...
    // Critical when an older afw (which only rewrote base_url) left the
    ...
    if (modelBaseUrl) {
      ...
      // routes.json (the config's base_url is now afw's URL). Fresh:
      ...
      const harvested = recorded?.harvest
        ? recorded.harvest
        : modelName && modelName !== AFW_MODEL_ID
          ? { id: modelName }
          : undefined

      endpoints.push({
        // Naming convention: `afw-hermes-<id>`. Single section so id
        ...
        modelId: `afw-${AGENT}-${providerName}`,
    ...
    if (customSeq instanceof YAMLSeq) {
      for (const item of customSeq.items) {
        ...
        const modelName = String(item.get('model') ?? item.get('default_model') ?? '').trim()
        const wrappedModelId = `afw-${AGENT}-${name}`
        const recorded = isAlreadyWired
    ...
    if (mcpMap instanceof YAMLMap) {
      for (const pair of mcpMap.items) {
        ...
        const args = argsSeq instanceof YAMLSeq ? (argsSeq.toJSON() as string[]) : undefined
    ...
    if (endpoints.length > 0) {
      const active = pickActiveEndpoint(endpoints, String(doc.getIn(['model', 'provider']) ?? ''))
      if (active) active.active = true
    }

    // Capture each provider's api_key so the orchestrator can inject it for
    // routes afw now manages.
  ...
  async wire(detection: Detection): Promise<WireOutcome> {
    ...
    // afw; the model-name field in the same section is overwritten with
    // `afw` so every Hermes request to this endpoint carries a model
    // name afw owns. The route key carries the real routing decision
    ...
    for (const ep of detection.endpoints) {
      ...
      // Write the wrap id (e.g. `afw-hermes-custom`) — that's the
      // model name hermes will send in body.model, which afw's
      // routes.json keys by. The legacy literal `'afw'` was the
      // old single-instance handle and no longer matches any route.
      const wrapModelId = ep.modelId
      if (ep.configLocation === '/model/base_url') {
        doc.setIn(['model', 'base_url'], ep.afwBaseUrl)
        ...
        changes.push({
          ...
          ...(priorModel === undefined ? { fromAbsent: true as const } : { from: priorModel }),
          to: wrapModelId,
        })
