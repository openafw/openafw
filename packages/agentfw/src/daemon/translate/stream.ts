...
import { canonicalStopReason, type IRResponse, type IRUsage, openaiStopReason } from './ir.ts'
...
async function readOpenAIResponses(
  stream: ReadableStream<Uint8Array>,
  emit: Emit,
): Promise<void> {
        ...
        for (const idx of this.order) {
          ...
          if (entry.block.type === 'text') {
            output.push({
              ...
              role: 'assistant',
              content: [
                { type: 'output_text', text: entry.block.text, annotations: [] },
              ],
            })
