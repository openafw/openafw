...
//
// agentfw never calls the provider's `/v1/models` for discovery — the
...
/** The decoder kinds agentfw can translate — others can't be a routing target. */
...
export async function seedFromRoutes(): Promise<void> {
  try {
    ...
    for (const [routeKey, entry] of Object.entries(routes)) {
      ...
      // agent processes can still proxy through agentfw. They must not seed
    ...
    await mutateModelRegistry((reg) => {
        ...
        if (!existing) {
          ...
          if (
            ...
            providers[providers.indexOf(existing)] = {
              ...
              auth: want.auth,
              ...(needsEffortBackfill
                ? { reasoningEffort: want.reasoningEffort }
                : {}),
            }
            changed = true
          }
      ...
      for (const want of desiredModels) {
        ...
        const existing = models.find(
          (m) => m.id === want.id && m.providerId === want.providerId,
        )
        if (!existing) {
          models.push(want)
          seen.add(want.id)
      ...
      const pruned = pruneVanishedSeeds(
        providers,
        models,
        desiredProviderIds,
        harvestOnlyDesired,
      ...
      return changed
        ? { ...reg, providers: pruned.providers, models: pruned.models }
        : undefined
    })
  } catch (err) {
...
async function appendObservedModel(
  agent: string,
  api: ModelApi,
  modelId: string,
): Promise<void> {
