...
export const updateCommand = new Command('update')
  .description('Update agentfw to the latest version — backed up, health-gated, auto-rolled-back on failure.')
  ...
  .action(async (opts: { check: boolean; force: boolean }) => {
    ...
    try {
      ...
      logger.print('Daemon not reachable. Start it with `agentfw daemon`, then retry.')
      process.exitCode = 1
      return
    }

    const { currentVersion, latestVersion, available, error } = info.state
    logger.print(`Installed: agentfw v${currentVersion}`)
    ...
    logger.print(`Available: agentfw v${latestVersion}`)
    if (opts.check) {
      logger.print('Run `agentfw update` to install it.')
      return
    }

    logger.print('')
    logger.print(`Updating v${currentVersion} → v${latestVersion}`)
    logger.print('agentfw backs up your database, health-checks the new version,')
    ...
    logger.print('')
    if (stage === 'stable') {
      logger.print(`✓ Updated to agentfw v${latestVersion}.`)
      if (!info.config.autoUpdateAsked) {
        ...
        logger.print(
          yes
            ...
            : 'Left off — run `agentfw update` whenever you want the next release.',
        )
      }
      ...
      logger.print('dashboard, or re-run `agentfw update` later to see the result.')
    }
  })
