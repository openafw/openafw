...
export function formatHtml(detail: RunDetail, opts: { redact: boolean }): string {
  ...
  out.push(`<title>agentfw trace — ${esc(r.id)}</title>`)
  out.push(`<style>${STYLE}</style>`)
  out.push('</head><body>')
  out.push(`<h1>agentfw trace <code>${esc(r.id)}</code></h1>`)
  ...
  out.push(
    `<footer>Captured by <a href="https://github.com/agentfw-com/agentfw">agentfw</a> — <code>agentfw report ${esc(r.id)} --format html</code></footer>`,
  )
  out.push('</body></html>')
  return `${out.join('\n')}\n`
}
