// First-run onboarding. afw taps the wire but does nothing useful until it
// knows where to send traffic, so the first time a user launches an agent (or
// runs `afw onboard`) we walk them through picking a model provider + model
...
export async function runOnboarding(agent: AgentId): Promise<boolean> {
  ...
  logger.print('')
  logger.print("Welcome to afw — let's set up where this agent's traffic should go.")
  logger.print('afw sits on the wire between your agent and its model provider: it sees')
  ...
  logger.print('`afw model` / `afw route`, or in the dashboard.')
  logger.print('')

  const choice = await promptChoice('How should afw route this agent?', [
    CONFIGURE,
  ...
  if (choice !== CONFIGURE) {
    ...
    logger.print(
      '\n✓ Passthrough selected — afw captures and guards traffic without swapping the model.',
    )
    return true
  }
  ...
  logger.print(`  Change it later with \`afw route set ${agent}/* --model <id>\` or the dashboard.`)
  return true
}
