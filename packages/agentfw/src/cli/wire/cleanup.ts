// Symmetric cleanup for `agentfw unwire`. Wire writes to five state stores
...
//
// Two variants:
//   • tombstoneAgentWireState — default `agentfw unwire` path (daemon stays
//     up). Marks routes tombstoned (proxy still serves them so live agent
//     processes can keep proxying through agentfw until they restart),
//     prunes the registry / routing-policy entries as if the routes were
//     gone, and **removes the captured secrets** so agentfw no longer
...
//     credentials in `~/.agentfw/secrets.json`.
//   • cleanupAgentWireState — `agentfw unwire --stop-daemon` path. Hard-
...
//     as `agentfw route set <key> --passthrough`).
...
/** Soft variant for the default `agentfw unwire` path (daemon stays up).
 *  Marks the agent's routes tombstoned instead of deleting them, so any
 *  already-running agent process can keep proxying through agentfw until
 ...
 *  secrets are also deleted — agentfw should not hold an unwired agent's
...
export async function tombstoneAgentWireState(agent: string): Promise<CleanupReport> {
  const report: CleanupReport = { ...EMPTY, routes: [], secrets: [], providers: [], models: [], routings: [] }
...
export async function cleanupAgentWireState(agent: string): Promise<CleanupReport> {
  ...
  const surviving = new Set(
    Object.keys(existing.routes).filter((k) => !toRemove.includes(k)),
  )
