...
/** The shape of the credential agentfw captured for a route at wire time.
 *  A static key's value is stored in secrets.json under
 *  `provider:<routeKey>`; an `agent-oauth` route carries no stored value —
 *  agentfw reads and refreshes the token from the agent's own credential
 *  store. */
...
export type RouteEntry = {
   ...
   *  agentfw-managed virtual handle and sourceModelId is the real
