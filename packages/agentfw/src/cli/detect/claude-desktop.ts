...
// chars). The value is never used for actual auth — agentfw strips the
// client header and re-injects the real credential from secrets.json
// before forwarding upstream.
const PLACEHOLDER_API_KEY =
  'sk-ant-api03-agentfw-managed-placeholder-substitute-downstream-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'
...
export const claudeDesktopDetector: Detector = {
  ...
  async detect(): Promise<Detection | null> {
    ...
    if (await fileExists(mcpPath)) {
      ...
      if (alreadyWrapped > 0) {
        caveats.push(`${alreadyWrapped} MCP server(s) already wrapped by agentfw-tap.`)
      }
      if (httpSkipped > 0) {
    ...
    // to come from agentfw. The user's already-wired Claude Code is the
    // natural source: same provider, same auth shape. agent-oauth reuses
    // the live token; api-key/bearer copies the value into a agentfw-owned
  ...
  async wire(detection: Detection): Promise<WireOutcome> {
    ...
    // Inject the agentfw tool surface (web_search etc.) as an MCP server
    ...
    // if missing.
    const toolsEntry = await injectAgentfwToolsMcp(AGENT, paths.agent.claudeDesktop.mcpConfig)
    if (toolsEntry) {
  ...
  manualInstructions(detection: Detection): string {
    ...
    const url = ep?.agentfwBaseUrl ?? agentfwUrlFor(AGENT)
    return [
      ...
      '          must look like a real sk-ant-api03-* key. agentfw strips',
      ...
      '     agentfw serves the model list from /v1/models.',
      '',
