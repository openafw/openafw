...
export type LaunchWiring = {
   ...
   *  carries afw-derived env vars (e.g. CLAUDE_CODE_AUTO_COMPACT_WINDOW)
...
const CODEX: LaunchWiring = {
  ...
  // config.toml per-invocation. Register an `afw` provider inline and
  // select it. Mirrors codex.ts's AFW_PROVIDER_DEFAULTS.
  async build(baseUrl) {
    const argvPrefix = [
      '-c',
      'model_provider=afw',
