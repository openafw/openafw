...
// agentfw reads that token to authenticate routes targeting Claude Code's
...
// next read instead of failing with refresh_token_reused. This makes agentfw
...
async function keychainAccount(): Promise<string | undefined> {
  try {
    ...
    const m = `${stderr}${stdout}`.match(
      /"acct"<blob>=(?:0x[0-9A-Fa-f]+\s+)?"([^"]*)"/,
    )
    return m?.[1]
  } catch {
    return undefined
...
function keychainStore(): ClaudeStore {
  ...
  return {
    ...
    async write(creds) {
      ...
      if (account === '') {
        logger.warn(
          'oauth: claude-code keychain account unknown; skipped token write-back',
        )
        return
      }
      try {
        ...
        logger.warn(
          `oauth: claude-code keychain write-back failed — ${(err as Error).message}`,
        )
      }
    },
  }
