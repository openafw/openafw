 ...
 *  `afw-openclaw-<agentId>`. Single-agent and multi-agent setups both
 *  use this — single-agent is just `afw-openclaw-main`. */
...
// why: afw wires by ADDING a new provider entry rather than mutating
// the user's existing one. The wrapper provider mirrors the source
// (baseUrl→afw, api/apiKey/models[] copied), then we re-aim
// `agents.defaults.model.primary` at "afw/<modelId>" so openclaw's
...
const WRAPPER_PROVIDER = 'afw' as const
...
// "afw/afw — afw is managing the model choice". The real source
// model id (e.g. "og-coding") is preserved in routes.json's `models[]`;
// proxy's swapVirtualModelForPassthrough swaps "afw" → that id at the
// wire so the upstream sees the model it actually serves.
const WRAPPER_MODEL_ID = 'afw' as const
const WRAPPER_MODEL_NAME = 'afw' as const
...
type OpenClawConfig = {
  ...
  // configs put a bare string at `agents.defaults.model`. afw reads
  // this to know which model the user is actively using — that's the one
  // we wrap behind the synthetic `afw` provider.
  agents?: {
    defaults?: {
  ...
  // `afw:default` mirroring the source's profile so openclaw's auth
...
type WrapTarget = {
  ...
  /** The base URL afw should forward to (the real upstream). */
...
type OpenClawProvider = {
  ...
  // byte-identical to afw's HarvestedModel — harvested at wire time.
  models?: unknown
  [k: string]: unknown
}
 ...
 *  afw-authored section in openclaw.json (the `afw` provider + its
...
/** Mirror the source provider's auth profile under `afw:default` in the
...
export const openclawDetector: Detector = {
  ...
  async detect(): Promise<Detection | null> {
    ...
    if (alreadyWired) {
      // Re-wire idempotent path: the user's primary is `afw/<modelId>`.
      ...
      if (recorded?.upstream) {
        ...
        caveats.push(
          `already wired through the '${WRAPPER_PROVIDER}' wrapper; re-wire is idempotent.`,
        )
      } else {
        caveats.push(
          `primary already aims at '${WRAPPER_PROVIDER}/*' but routes.json has no openclaw/${WRAPPER_PROVIDER} entry — run \`afw unwire openclaw\` then re-wire to re-init.`,
        )
      }
    } else if (target) {
      // Fresh wire: wrap the source provider+model for EACH openclaw
      // sub-agent. The source provider entry stays byte-identical; wire()
      // adds a sibling `afw` provider whose models[] has one entry
      // per agent (id = `afw-openclaw-<agentId>`). Per-agent auth
      // profiles get their own `afw:default` entries (potentially
      // different keys per agent).
      if (config.models?.providers?.[WRAPPER_PROVIDER]) {
        caveats.push(
          `models.providers.${WRAPPER_PROVIDER} already exists but primary doesn't point at it — \`afw unwire openclaw\` first to clean it up.`,
        )
      } else {
      ...
      caveats.push(
        currentPick
          ...
          : `agents.defaults.model.primary not set — point it at the model you want afw to wrap, then wire.`,
      )
    }
  ...
  async wire(detection: Detection): Promise<WireOutcome> {
    ...
    // afw-authored block to clean (the `afw` provider + primary +
    ...
    // 1. models.providers.afw — the wrapper. ONE provider entry with
    ...
    // One wrapper-model entry per endpoint. id = afw-openclaw-<agentId>;
    ...
    const wrapperModelEntries: Record<string, unknown>[] = eps.map((ep) => {
      ...
      return {
        ...
        id: ep.modelId,
        name: `afw (${openClawAgentId})`,
      }
    })
    const ep0 = eps[0]!
    const wrapperEntry: Record<string, unknown> = {
      [target.baseUrlKey]: ep0.afwBaseUrl,
      ...(typeof target.sourceProvider.api === 'string' ? { api: target.sourceProvider.api } : {}),
    ...
    // 2. agents.defaults.models["afw/<wrapperModelId>"] for each agent —
    ...
    for (const ep of eps) {
      ...
      if (priorActive === undefined) {
        ...
        const sourceActive = tree
          ? (getValueAt<unknown>(tree, ['agents', 'defaults', 'models', sourceActiveKey]) ?? {})
          : {}
    ...
    // 3. agents.defaults.model.primary → "afw/<first-wrapper>" globally.
    ...
    // 4. auth.profiles["afw:default"] — mirror the source's profile
    ...
    const wrapperCreds = new Map<
      string,
      { auth: import('../../core/routes.ts').RouteAuth; value?: string }
    >()
    for (const ep of eps) {
      const openClawAgentId = ep.modelId.startsWith(`${WRAPPER_PROVIDER}-${AGENT}-`)
        ? ep.modelId.slice(`${WRAPPER_PROVIDER}-${AGENT}-`.length)
    ...
    // 6. Per-agent auth-profiles.json: write `afw:default` into EACH
