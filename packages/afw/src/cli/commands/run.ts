// `afw run` — the explicit/advanced launcher. Launch one agent instance
...
// `afw claude` / `afw codex` (see commands/agents.ts); both sit on the
...
export const runCommand = new Command('run')
  .description(
    'Launch one agent instance with its own wire identity — per-instance capture ' +
      ...
      '    afw run --as solo --raw -- claude                        # bypass afw',
  )
  ...
  .option('--raw', 'bypass afw entirely for this instance (no capture, no routing)')
  ...
  .action(async (command: string[], opts: RunOpts) => {
    ...
    if (command.length === 0) {
      fail('no command to launch — pass it after `--`, e.g. `afw run -- claude`')
    }
    ...
    if (!wiring) {
      fail(
        `afw run can't wrap "${opts.agent ?? basename(bin!)}" yet — it supports launch-per-task agents (claude, codex). For daemon agents like OpenClaw, run \`afw openclaw\` for setup steps.`,
      )
    }
