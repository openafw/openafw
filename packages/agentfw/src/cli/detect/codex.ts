...
const AGENTFW_PROVIDER_ID = 'agentfw'
...
const AGENTFW_PROVIDER_DEFAULTS: Record<string, string | { raw: string }> = {
  name: 'agentfw (OpenAI)',
  wire_api: 'responses',
...
// '/chat/completions') to the configured base_url. The agentfw proxy
...
function agentfwBaseUrl(): string {
  return agentfwUrlFor(AGENT)
}
...
export const codexDetector: Detector = {
  ...
  async detect(): Promise<Detection | null> {
    ...
    if (currentProvider === AGENTFW_PROVIDER_ID) {
      caveats.push('config.toml already points at agentfw; re-wire is idempotent.')
      ...
      const customBaseUrl = getTomlString(
        text,
    ...
    const ep: PlannedEndpoint = {
      ...
      agentfwBaseUrl: wireUrl,
      upstream,
      decoder: 'openai-responses',
      configLocation: `/model_providers/${AGENTFW_PROVIDER_ID}/base_url`,
      filePath: configPath,
      active: true,
    }

    // Capture the agent's static OpenAI key so the orchestrator can inject
    // it for routes agentfw now manages. ChatGPT-subscription auth is OAuth
  ...
  async wire(detection: Detection): Promise<WireOutcome> {
    ...
    if (originalProvider === AGENTFW_PROVIDER_ID) {
      const existing = getTomlString(
        originalText,
        `model_providers.${AGENTFW_PROVIDER_ID}`,
        'base_url',
      )
      if (existing === ep.agentfwBaseUrl) {
    ...
    // 1. Top-level model_provider = "agentfw"
    const topRes = setTomlTopLevelString(working, 'model_provider', AGENTFW_PROVIDER_ID)
    working = topRes.text

    // 2. [model_providers.agentfw] section with our defaults + wire URL.
    const desired: Record<string, string> = {
      ...AGENTFW_PROVIDER_DEFAULTS,
      base_url: ep.agentfwBaseUrl,
    }
    const secRes = ensureTomlSection(
      working,
      `model_providers.${AGENTFW_PROVIDER_ID}`,
      desired,
    )
    ...
    // The whole [model_providers.agentfw] section is agentfw-owned. Record
    ...
    const changes: ChangeRecord[] = [
      rawProvider === undefined
        ? { type: 'set', jsonPointer: '/model_provider', fromAbsent: true, to: AGENTFW_PROVIDER_ID }
        : {
            type: 'set',
            jsonPointer: '/model_provider',
            from: rawProvider,
            to: AGENTFW_PROVIDER_ID,
          },
    ]
    if (secRes.sectionAdded) {
      changes.push({
        type: 'create-path',
        jsonPointer: `/model_providers/${AGENTFW_PROVIDER_ID}`,
      })
    }
