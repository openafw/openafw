// `agentfw model add` — interactive, multi-provider model onboarding. Prompts
...
// run. The flag-based `agentfw route provider/model add` stays for scripting.
...
const listCmd = new Command('list')
  ...
  .action(async () => {
    try {
      const reg = await daemonFetch<{ providers: ProviderEntry[]; models: ModelEntry[] }>(
        'GET',
...
export const modelCommand = new Command('model')
  .description('Manage the providers, models, and API-key secrets agentfw can route to.')
  .addCommand(addCmd)
  .addCommand(listCmd)
  .addCommand(modelRm)
