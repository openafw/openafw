...
// User preferences, stored at ~/.afw/config.json. Distinct from
...
export async function readConfig(): Promise<AfwConfig> {
  try {
    const parsed = JSON.parse(await readFile(paths.config, 'utf8')) as Partial<AfwConfig>
...
export async function updateConfig(patch: Partial<AfwConfig>): Promise<AfwConfig> {
