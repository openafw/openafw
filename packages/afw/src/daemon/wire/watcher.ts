...
// ~/.afw/backups/manifest.json) and flags semantic drift — i.e. when
...
//
// We only DETECT drift here. Re-wiring is a user action (`afw wire`).
 ...
 * with `jsonPointer` like '/model_providers/afw/base_url' or
    ...
    if (Array.isArray(current)) {
      ...
      if (Number.isFinite(idx) && String(idx) === seg) {
        ...
        current = current.find(
          (x) => x != null && typeof x === 'object' && (x as { name?: unknown }).name === seg,
        )
      }
