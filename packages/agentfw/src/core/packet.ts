...
export type ModelCallPayload = {
   ...
   *  before any agentfw-side swap. Differs from `model` when:
   ...
   *      claude-desktop sends `claude-opus-4-7` and agentfw routes to
   *      `deepseek-v4-pro`).
...
/** A tool executed locally by agentfw — today that's web_search emulation
 ...
 *  agentfw ran it via DDG" at a glance — without forcing the user to dig
 *  into the next-round messages.json. */
export type ToolCallPayload = {
  kind: 'tool_call'
  /** Tool name as the model called it (e.g. 'web_search'). */
  toolName: string
  /** ToolProvider id from ~/.agentfw/tool-providers.json. */
  providerId: string
...
export type AgentPacket = {
   ...
   *  raw_res. They are the truth for `agentfw show --raw` and deterministic
   *  `agentfw replay`. Absent on paths that don't capture raw (orchestrator
   *  fan-out, passthrough). */
  rawReq?: Uint8Array
  rawRes?: Uint8Array
