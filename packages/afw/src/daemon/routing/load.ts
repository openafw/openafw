...
import { type AccessKeys, EMPTY_ACCESS_KEYS, readAccessKeys } from '../../core/access-keys.ts'
import { logger } from '../../core/logger.ts'
import { EMPTY_REGISTRY, type ModelRegistry, readModelRegistry } from '../../core/model-registry.ts'
import { paths } from '../../core/paths.ts'
import { EMPTY_POLICY, type RoutingPolicy, readRoutingPolicy } from '../../core/routing-policy.ts'
import { EMPTY_SECRETS, type SecretStore, readSecrets } from '../../core/secrets.ts'
import { EMPTY_TIERS, type TierConfig, readTiers } from '../../core/tiers.ts'

let registry: ModelRegistry = EMPTY_REGISTRY
let policy: RoutingPolicy = EMPTY_POLICY
let secrets: SecretStore = EMPTY_SECRETS
let accessKeys: AccessKeys = EMPTY_ACCESS_KEYS
let tiers: TierConfig = EMPTY_TIERS
...
const KEYS_FILE = basename(paths.keys)
const TIERS_FILE = basename(paths.tiers)
...
export function getAccessKeys(): AccessKeys {
  return accessKeys
}
...
export async function initRoutingTables(): Promise<void> {
  await Promise.all([reloadModels(), reloadRouting(), reloadSecrets(), reloadKeys(), reloadTiers()])
  startWatcher()
}
...
async function reloadKeys(): Promise<void> {
  try {
    accessKeys = await readAccessKeys()
    logger.debug(`routing: loaded ${accessKeys.keys.length} access key(s)`)
  } catch (err) {
    logger.error(`routing: failed to load keys.json — ${(err as Error).message}`)
  }
}

async function reloadTiers(): Promise<void> {
  try {
    tiers = await readTiers()
    logger.debug(`routing: loaded ${Object.keys(tiers.tiers).length} tier mapping(s)`)
  } catch (err) {
    logger.error(`routing: failed to load tiers.json — ${(err as Error).message}`)
  }
}
...
function startWatcher(): void {
  if (watcher) return
  try {
    // All three files live directly under ~/.afw/ — one directory watch
    ...
    logger.warn(`routing: watch failed — ${(err as Error).message}; reload won't be automatic`)
  }
}
