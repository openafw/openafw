...
// response. When afw routes that request to a non-Anthropic
...
//
// This module makes afw the executor instead:
//
...
//     && member.api === 'anthropic-messages'). Cross-protocol uses afw's
 ...
 *  loop so afw serves the search instead of the agent's own runtime
  ...
  for (const b of blocks) {
    ...
    out.push({
      ...
      ...(Array.isArray(input.allowed_domains)
        ? {
            allowed_domains: input.allowed_domains.filter(
      ...
      ...(Array.isArray(input.blocked_domains)
        ? {
            blocked_domains: input.blocked_domains.filter(
              (s): s is string => typeof s === 'string',
            ),
          }
        : {}),
...
function anthropicAssistantContent(memberApi: ModelApi, json: unknown): unknown[] | undefined {
...
export async function runWebSearchEmulationLoop(
  ...
  const messages = ((params.clientRequest.messages as unknown[]) ?? []).slice()
  ...
  for (let iter = 0; iter < maxUses; iter++) {
    ...
    const toolResults = await Promise.all(
      calls.map(async (call) => {
        ...
        toolExecutions.push(buildToolExecutionRecord(backend, call, outcome, ts, durMs))
...
function buildToolExecutionRecord(
  ...
  const costUsd = backend?.costPerCall && backend.costPerCall > 0 ? backend.costPerCall : undefined
  if (!outcome.ok) {
    return {
      ts,
      durMs,
