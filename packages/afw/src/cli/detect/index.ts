...
/**
 * Order matters for UX (output order in `afw wire`). High-confidence
 * detectors first.
 */
...
export async function detectAll(opts: { only?: AgentId[] } = {}): Promise<Detection[]> {
  const targets = opts.only ? DETECTORS.filter((d) => opts.only?.includes(d.agent)) : DETECTORS
