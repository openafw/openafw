...
// companion configured, agentfw calls the companion *itself* before the
...
export async function preDescribeImages(
  ...
  const rewrittenBody = serializeRequestFromIR(clientApi, rewrittenIR) as Record<
    string,
    unknown
  >
...
export function requestHasImageBlock(clientApi: ModelApi, clientRequest: Record<string, unknown>): boolean {
  try {
