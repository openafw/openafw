...
const { clearBudgetCache, periodStart, spendInPeriod, tokensInPeriod } = await import('./budget.ts')
...
describe('periodStart', () => {
  ...
  it('returns now minus the span for a rolling window (the 5h subscription window)', () => {
    const now = new Date(2026, 4, 21, 15, 30, 0).getTime()
    expect(periodStart({ rollingHours: 5 }, now)).toBe(now - 5 * 3_600_000)
  })
})
