...
//
// No file I/O, no MCP coupling, no daemon dependency — the afw-tools
  ...
  try {
    res = await fetch(url, {
      ...
      headers: {
        ...
        // OpenClaw uses 'openclaw'; we identify as 'afw' so the
...
function freshnessToFilter(freshness: string | undefined): Record<string, unknown> | undefined {
