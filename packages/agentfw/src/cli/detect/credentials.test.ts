...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'agentfw-creds-'))
})
...
function ep(
  ...
  return {
    ...
    agentfwBaseUrl: 'http://localhost:9877/wire/x/y',
...
describe('buildWireSecrets', () => {
  it('emits one ref per active endpoint with a captured credential', () => {
    ...
    const secrets = buildWireSecrets(
      'hermes',
      [ep('openai', '/model/base_url'), ep('groq', '/custom_providers/groq/base_url', 'openai-chat', false)],
      captured,
    )
