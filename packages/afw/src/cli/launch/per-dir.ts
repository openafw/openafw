// Per-directory launch memory. `afw claude` in a project remembers the
...
// centrally under ~/.afw/launch/ keyed by the absolute cwd — nothing is
...
import { AFW_HOME } from '../../core/paths.ts'

export type LaunchConfig = {
  /** Route this instance to a single model. */
  model?: string
  /** 'monitor' = capture but never reroute; 'raw' = bypass afw entirely. */
  mode?: 'monitor' | 'raw'
...
const launchDir = (): string => join(AFW_HOME, 'launch')
