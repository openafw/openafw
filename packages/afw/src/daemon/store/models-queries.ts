 ...
 * actually flowing through afw so they know what to price.
 */
export async function recentModels(sinceMs: number): Promise<RecentModelRow[]> {
  ...
  const rows = db
    ...
    .where(and(eq(actions.kind, 'model_call'), gte(actions.ts, sinceMs), isNotNull(actions.model)))
 ...
 */
export async function baselineInputTokens(agent: string, sinceMs: number): Promise<number | null> {
  const db = await getDb()
  const row = db
