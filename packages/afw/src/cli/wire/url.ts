...
/** afw's wire URL for an agent — single path segment carrying the
 *  agent type. Body.model carries per-instance dispatch. */
export function afwUrlFor(agent: AgentId): string {
  return `${DAEMON_BASE_URL}/wire/${agent}`
}

/** afw's wire URL for one agent *instance* — the agent segment carries an
 *  `@<instance>` suffix minted per launch by `afw run`. The proxy
...
export function afwUrlForInstance(agent: AgentId, instanceId: string): string {
  return `${DAEMON_BASE_URL}/wire/${agent}@${instanceId}`
}

export function afwMcpUrlFor(agent: AgentId, mcpName: string): string {
