 ...
 * currently-running agentfw process was invoked so dev (raw .ts via
 * --experimental-strip-types) and prod (built .js) both work.
 */
function computeDaemonArgs(): string[] {
  const node = process.execPath
  const entry = process.argv[1]
  if (!entry) throw new Error('cannot resolve agentfw entry path (process.argv[1] is empty)')
