...
-- (the agent runs them locally; afw sees them on the model wire). One
...
function ensureColumns(
  ...
  const existing = raw.prepare(`PRAGMA table_info(${table})`).all() as { name: string }[]
...
function migratePayloadSidecar(raw: Database.Database): void {
  ...
  logger.info(`store: payload sidecar migration done in ${(performance.now() - t0).toFixed(0)}ms`)
}
...
function backfillModelColumn(raw: Database.Database): void {
  const needsBackfill = raw
    .prepare("SELECT 1 FROM actions WHERE kind = 'model_call' AND model IS NULL LIMIT 1")
    .get() as unknown
...
export function applySchema(raw: Database.Database): void {
  ...
  raw.exec('CREATE INDEX IF NOT EXISTS actions_kind_model_ts ON actions(kind, model, ts)')
  ...
  // This covering index carries every small column the /api/afw
  ...
  raw.exec('CREATE INDEX IF NOT EXISTS tool_uses_action_id ON tool_uses(action_id)')
  ...
  raw.exec('CREATE INDEX IF NOT EXISTS runs_thread_started ON runs(thread_id, started_at)')
...
function backfillHttpStatus(raw: Database.Database): void {
  const needsBackfill = raw
    .prepare("SELECT 1 FROM actions WHERE kind = 'model_call' AND http_status IS NULL LIMIT 1")
    .get() as unknown
  if (!needsBackfill) return
  ...
  logger.info(`store: http_status backfill done in ${(performance.now() - t0).toFixed(0)}ms`)
}
