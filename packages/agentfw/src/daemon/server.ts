...
export async function startServer(opts: { port: number }): Promise<void> {
  ...
  // dispatch (wrap-style: `agentfw-openclaw-main`; OAuth: real model id
  // like `claude-opus-4-7`). Proxy looks up `findRoute(agent, modelId)`.
  //
  // CORS: agents that fetch through agentfw from a browser/Electron
...
<body>
  ...
  <p>The daemon is running. Use the CLI: <code>agentfw status</code>, <code>agentfw route</code>.</p>
</body>
</html>`
