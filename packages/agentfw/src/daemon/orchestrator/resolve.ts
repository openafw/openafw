...
export function resolveRoute(routeKey: string, decoder: DecoderKind): ResolvedRoute {
  ...
  // Resolve the (members, capabilities) inputs. A `composite` target
  // dereferences a registry combination model — the combo owns its members and
  // capabilities, so the route's own `capabilities` are ignored. A `chain`
  // target uses its inline members and the route's per-route capabilities.
  let memberSpecs: ChainMember[]
  let rawCaps: Partial<Record<CapabilityId, CapabilityFulfillment>> | undefined
  if (target.kind === 'composite') {
    ...
    memberSpecs = combo.members
    rawCaps = combo.capabilities
  } else {
    memberSpecs = target.members
    rawCaps = routing.capabilities
  }

  const capabilities = resolveCapabilityMap(routeKey, rawCaps)
