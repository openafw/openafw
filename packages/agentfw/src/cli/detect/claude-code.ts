...
export const claudeCodeDetector: Detector = {
  ...
  async detect(): Promise<Detection | null> {
    ...
    if (settingsExists) {
      ...
      if (isAlreadyWired) {
        caveats.push('settings.json already points at agentfw; re-wire is idempotent.')
      }
    }
    ...
    if (legacyExists) {
      ...
      for (const [name, server] of Object.entries(servers)) {
        ...
        // Idempotency: stdio wraps via agentfw-tap; http/sse repoint url at the relay.
      ...
      if (alreadyWrapped > 0) {
        caveats.push(`${alreadyWrapped} MCP server(s) already wired by agentfw.`)
      }
    }

    // Capture the agent's own credential so the orchestrator can inject it
    // for cross-agent routes (and same-provider routes agentfw now manages).
  ...
  async wire(detection: Detection): Promise<WireOutcome> {
    ...
    if (ep && ep.originalBaseUrl !== ep.agentfwBaseUrl) {
    ...
    // 3. Inject mcpServers.agentfw so the agent picks up agentfw's local
    //    tool surface (web_search, web_fetch, …) regardless of routing.
    //    Idempotent + file-creating; same machinery as claude-desktop.
    const toolsEntry = await injectAgentfwToolsMcp(AGENT, paths.agent.claudeCode.legacy)
    if (toolsEntry) {
...
async function wireEnvRedirect(ep: PlannedEndpoint): Promise<BackupEntry> {
  ...
  // own base_url" apart from "delete the key agentfw added".
  ...
  const newText = applyJsonUpdate(originalText, ['env', 'ANTHROPIC_BASE_URL'], ep.agentfwBaseUrl)
  ...
  changes.push(
    priorBaseUrl === undefined
      ? { type: 'set', jsonPointer: ep.configLocation, fromAbsent: true, to: ep.agentfwBaseUrl }
      : { type: 'set', jsonPointer: ep.configLocation, from: priorBaseUrl, to: ep.agentfwBaseUrl },
  )
