...
const INSTRUCTION_OVERRIDE: Array<{ re: RegExp; why: string }> = [
  { re: /\bignore\s+(?:all\s+)?(?:the\s+)?(?:previous|above|prior|earlier|preceding)\b/i, why: 'ignore-previous' },
  { re: /\bdisregard\s+(?:all\s+)?(?:the\s+)?(?:previous|above|prior|system)\b/i, why: 'disregard' },
  { re: /\b(?:forget|override)\s+(?:everything|all|your|the)\b.{0,40}\b(?:instruction|prompt|rule)/i, why: 'override-instructions' },
...
const EXFIL: Array<{ re: RegExp; why: string }> = [
  { re: /\b(?:send|post|upload|exfiltrate|leak)\b.{0,40}\b(?:api[_\s-]?key|secret|token|credential|password|\.env)\b/i, why: 'exfil-secrets' },
...
function scan(text: string): Hit[] {
  ...
  for (const p of INSTRUCTION_OVERRIDE) if (p.re.test(text)) add('prompt-injection:instruction-override', 'high', p.why)
  for (const p of ROLE_MARKER) if (p.re.test(text)) add('prompt-injection:role-injection', 'high', p.why)
